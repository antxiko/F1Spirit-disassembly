...
OPCION_1_ELEGIDA:		; E240 = 0, E27F = 2 (tres opciones), E27E = 0 y E251++ (0x6079); cursor de p03 0xBBAB a EA88; borra los textos p03 0xAB51 y 0xAB59 y pinta el 0xAB5E
	...
	call SIGUIENTE_PASO_MENU		;6027   ; y se pasa al paso siguiente del menu
	ld de,0ea88h		;602a
...
PINTA_MENU_CATEGORIAS:		; la raya vertical (0x613A), E27E = 0 (0x619E), la lista de textos con paginas 5/6 (0x62D4), E27F = 2, cursor de p03 0xBBAF a EA88, borra el texto 0xAB5E, pinta 0xAB74; si pasa los puntos: 0xAB87 (E27F = 4) y 0xAB93 (E27F = 5); E35D = GP maximo (0x61C7) y lo pinta en 0x3A7B
	...
	ld a,002h		;608b
	ld (0e27fh),a		;608d   ; tres opciones tambien en este menu
	...
	call PINTA_NUMERO_2_DIGITOS		;60ce   ; dos cifras: los puntos que hacen falta
...
PINTA_GP_ELEGIDO:		; FILVRM 13 ceros en 0x3AB3; numero E29D+1 en 0x3ADC (0x617E); el flujo de tiles 0x635A[E29D] en 0x3AB3 (p00 0x4807)
	...
	call PINTA_NUMERO_2_DIGITOS		;60e1   ; dos cifras: el numero del GP
	ld a,(0e29dh)		;60e4
	ld de,0635ah		;60e7   ; tabla de 16 punteros a flujos de tiles (0x6E37..0x6ECC)
	call 04a3bh		;60ea   ; 0x4A3B es el lector de palabras de tabla de p00
	ld hl,03ab3h		;60ed
...
MENU_CATEGORIA:		; E251 = 2: cursor por p03 0xBBD1 (0x626C), textos (0x61A2, 0x62D4), si E27E = 5 mueve el GP (0x623C); con el boton: E25B = categoria de E27E (0x6128), E25C = E25B o 5 + E29D, E25F = 0 y E250 = 6
	...
	ret z			;610a   ; sin boton no hay eleccion
	ld hl,0e29dh		;610b
	ld a,(0e27eh)		;610e
	call CATEGORIA_DE_OPCION		;6111   ; tabla 0x6135: opcion del menu -> categoria (1,0,3,4,2)
	ld (0e25bh),a		;6114   ; y la categoria elegida se apunta en E25B
	cp 005h		;6117
...
CATEGORIA_DE_OPCION:		; si A < 5: A = tabla 0x6135[A]; si no A tal cual
	...
	push hl			;612b
	ld hl,06135h		;612c
	call 040d0h		;612f   ; 0x40D0 es el SUMA_A_HL de p00
	ld a,(hl)			;6132
	pop hl			;6133
	ret			;6134
...
PINTA_NUMERO_2_DIGITOS:		; A+1 a BCD (p00 0x4B28) y sus dos digitos en VRAM HL y HL+1 como tiles 0x10+n (decena 0 = tile 0)
	...
	ld l,a			;6180
	ld h,000h		;6181
	call 04b28h		;6183   ; 0x4B28 es el paso de binario a decimal de p00
	pop hl			;6186
	ld a,e			;6187
	...
	add a,010h		;6190   ; 0x10 es el tile del cero
...
CURSOR_A_CERO:		; E27E = 0
	xor a			;619e
	ld (0e27eh),a		;619f   ; la opcion vuelve a la primera
...
NOMBRE_CATEGORIA_INDICE:		; A = categoria de la opcion (0x6128) y sigue
	...
	call 04a44h		;61b8   ; 0x4A44 lee la palabra de una tabla que esta en la pagina mapeada
	ld de,0e423h		;61bb   ; once columnas descomprimidas en E423: el hueco del nombre en el menu
	ld a,00bh		;61be
	ex de,hl			;61c0
	call RLE_A_BUFFER		;61c1
	jp 043feh		;61c4   ; 0x43FE vuelca el buffer a la VRAM
GP_MAXIMO:		; A = 0; por cada umbral de puntos superado (0x21,0x30,0x41,0x54,0x69,0x80,0x99,0xB4) A += 2; si pasa todos A = 15
	...
	ret c			;61ca   ; el primer umbral que no se pasa corta la escalera
...
PUNTOS_MENOS_0C:		; DE = 0x0C y compara (0x6227)
	ld de,0000ch		;61f7   ; cada umbral tiene su puerta, y todas caen en la misma comparacion
...
COMPARA_PUNTOS:		; si E1DF != 0: NZ y sin carry; si no carry solo si (E295) < DE y (E355) < DE (los 16 bits de ix-0x2B de los dos coches)
	...
	or a			;6231
	sbc hl,de		;6232
	ret nc			;6234   ; con los puntos por encima del umbral, carry a cero: abierto
...
MUEVE_CURSOR_FLANCOS:		; A = flancos: bit 0 arriba, bit 1 abajo
	...
	rra			;627a
	ret nc			;627b
	inc (hl)			;627c   ; hacia abajo, uno mas
...
CURSOR_A_POSICION:		; EA88/EA89 = el par E27E de la tabla DE; sonido 0x25 si E250 != 0x18
	...
	ex de,hl			;628f
	add a,a			;6290
	call 040d0h		;6291   ; 0x40D0 es el SUMA_A_HL de p00
	ld de,0ea88h		;6294
...
SONIDO_25:		; ENCOLA_SONIDO 0x25 (cursor)
	ld a,025h		;62a2
	jp 04174h		;62a4   ; 0x4174 es el que pide sonido en p00
...
BOTON_FLANCO:		; bit 4 de A
	...
	cp 018h		;62b9
	jr z,BOTON_SI		;62bb
	ld a,026h		;62bd   ; el 0x26 es el sonido de elegir
	call 04174h		;62bf
BOTON_SI:		; A = 0xFF, NZ
	or 0ffh		;62c2   ; y se vuelve con A distinto de cero: hay boton
	ret			;62c4
...
PINTA_TEXTOS_CATEGORIAS:		; p09 en A000; texto 0xAC0F -> EC02 -> E501 (7x11, fuente 0x30); 1/2/3; 0x6BF3 y 0x6CF4 (0x60 a E5AB/E5CB) por coche; y 0x631F para E52D (coche 1) y E54D (coche 2)
	...
	ld hl,0e5abh		;62f9
	ld b,060h		;62fc   ; B es la fuente que se usa para los digitos
...
VALOR_COCHE_LEE:		; HL = DE - 0x40 + A: el byte del coche
	...
	ld l,a			;6336
	ld h,000h		;6337
	call 04b28h		;6339   ; 0x4B28 pasa el binario a decimal
	pop hl			;633c
...
CARGA_TILES_MENU:		; listas de tiles p04 0x6D6B, 0x6C7C y 0x620A (CARGA_LISTA_TILES) y la lista de sprites p04 0x74EF
	...
	ld hl,06c7ch		;6348
	call 04ccdh		;634b   ; 0x4CCD es el cargador de listas de tiles de p00
	ld hl,0620ah		;634e
	call 04ccdh		;6351
	ld hl,074efh		;6354
	jp 04f11h		;6357   ; 0x4F11 es el cargador de la lista de sprites
...
RLE_BUFFER_COPIA:		; n bytes tal cual (n < 0x80)
	...
	djnz RLE_BUFFER_COPIA		;639e   ; tantos bytes como diga el codigo
...
RLE_BUFFER_MAS:		; codigos >= 0x81: repetir (< 0xE1), serie creciente (< 0xF1) o decreciente
	...
	res 7,b		;63a7   ; el bit 7 se quita: lo que queda es la cuenta
...
RLE_BUFFER_REPITE:		; el mismo byte n&0x7F veces
	...
	pop af			;63b6
	djnz RLE_BUFFER_REPITE		;63b7   ; tantas veces como diga la cuenta
...
RLE_BUFFER_SERIES:		; n = codigo - 0xE1: < 0x10 creciente, si no decreciente
	...
	inc b			;63c0   ; el `inc b` es porque la cuenta viene con uno menos
	ld a,(de)			;63c1
	inc de			;63c2
	ld c,a			;63c3
RLE_BUFFER_CRECIENTE:		; t, t+1, t+2... (n+1 valores)
	...
	inc c			;63cf   ; el tile sube de uno en uno
	djnz RLE_BUFFER_CRECIENTE		;63d0   ; tantos tiles como diga la cuenta
...
RLE_BUFFER_DECRECIENTE_BUCLE:		; un valor por vuelta
	...
	dec c			;63e4   ; y el tile baja de uno en uno
	djnz RLE_BUFFER_DECRECIENTE_BUCLE		;63e5   ; tantos tiles como diga la cuenta
...
PINTA_PANTALLA_COCHE:		; cursor HL (4 bytes) a EA88; borra E400 (0x21F bytes); VRAM (0x6635); tiles (0x6781); hasta 3 valores de la fila E251 de la tabla p03 0xBC45 (0x676C) pintados en E422/E42C/E436 (0x6611); 0xFF acaba
	ld de,0ea88h		;6417
	ld bc,00004h		;641a
	ldir		;641d   ; cuatro bytes: el atributo entero del cursor
	ld hl,0e400h		;641f   ; 0x21F bytes del buffer a cero: las 17 filas de arriba
	ld bc,0021fh		;6422
	call 04b87h		;6425   ; 0x4B87 es el borrado de memoria de p00
...
ELECCION_MANDO:		; flancos del mando activo: izquierda (bit 2) -> 0x6580, derecha (bit 3) -> 0x6599, boton (bit 4): sonido 0x26, guarda la eleccion en (0x67F0)+E251-1 y pasa al paso siguiente (tabla 0x67E4 de textos) o, en el paso 5, a 0x64AC
	...
	rra			;6476
	ret nc			;6477
	ld a,026h		;6478   ; el 0x26 es el sonido de elegir
	call 04174h		;647a
	...
	pop hl			;6491
	ld (hl),a			;6492   ; y el valor elegido se guarda
	ld a,(0e251h)		;6493
...
ELECCION_FIN:		; E251 = A; cursor p03 0xBBBB a EA88; E27E = 0, E27F = 1
	...
	ld bc,00004h		;64c5
	ldir		;64c8   ; cuatro bytes: el atributo entero del cursor
	xor a			;64ca
	ld (0e27eh),a		;64cb
	inc a			;64ce
	ld (0e27fh),a		;64cf   ; una sola opcion mas: si o no
	ret			;64d2
...
SIGUIENTE_JUGADOR:		; con dos jugadores (bit 5 de E1C2): E25F++ y si != 2 vuelve a E250 = 6; si no limpia (0x465B) y E250 = 0x0A
	...
	ld hl,0e25fh		;64df
	inc (hl)			;64e2   ; y le toca al otro jugador
	ld a,(hl)			;64e3
...
ELECCION_SI_NO:		; E251 >= 6: cursor por p03 0xBBE1 (0x6266); boton: E27E = 0 -> 0x64D8; si no borra 0xAC07, E251 = 0 y E250 = 6 (si E251 era 7) o textos 0xABFB/0xABE3 (0x6756)
	...
	ret z			;64fc   ; sin boton no hay respuesta
...
ELECCION_POR_E252:		; E252: 1 -> 0x6469; >= 2 -> 0x65DE; 0 -> cursor por p03 0xBBDD; boton: E27E != 0 -> textos 0xABAD/0xABD9 y 0x67CF; E27E = 0 -> borra 0xABAD, pinta 0xABCA, cursor 0xBBB3, E252++ y 0x67CF
	...
	ret z			;6534   ; sin boton no hay eleccion
...
ELECCION_OPCION_0:		; borra 0xABAD, pinta 0xABCA, cursor 0xBBB3, E252++, 0x67CF
	...
	ldir		;655c   ; cuatro bytes: el atributo entero
	ld hl,0e252h		;655e
	inc (hl)			;6561
...
COPIA_5_BYTES_COCHE:		; copia a (HL) los 5 bytes de la entrada (valor de p03 0xBC45 por E251/E27E) de la tabla p03 0xBBEB (5 bytes por entrada)
	...
	call 040d0h		;6576
	pop de			;6579
	ld bc,00005h		;657a   ; cinco bytes: el coche entero
	ldir		;657d
	ret			;657f
...
ELECCION_DER_CURSOR:		; EA89 == 0xE8: entra el panel; si no, EA89 += 0x50
	...
	ld c,050h		;65b1   ; 0x50 es lo que avanza el cursor de una ficha a la siguiente
...
ARRANCA_PANEL:		; E25D = C (lado), E25E = 10 (pasos), E252 = 2; borra E920.. (0xFF bytes); pinta en E821 el valor (E251, E27E) (0x6611); sonido 0x25
	...
	ld a,00ah		;65bc
	ld (0e25eh),a		;65be   ; diez pasos de deslizamiento
	ld a,002h		;65c1
	ld (0e252h),a		;65c3
	ld hl,0e920h		;65c6   ; 0xFF bytes de E920 a cero: el sitio donde se monta la ficha que entra
	ld bc,000ffh		;65c9
	call 04b87h		;65cc   ; 0x4B87 es el borrado de memoria de p00
	...
	jr SONIDO_25_B		;65dc   ; y suena el 0x25 al cambiar de ficha
...
DESLIZA_CUENTA:		; E25E--; al acabar E252 = 1 (0 si E251 = 0)
	ld hl,0e25eh		;65f8
	dec (hl)			;65fb
	ret nz			;65fc   ; hasta que se agoten los diez pasos
...
SONIDO_25_B:		; ENCOLA_SONIDO 0x25
	ld a,025h		;660c
	jp 04174h		;660e   ; el 0x25 es el sonido de mover el cursor
...
VUELCA_512_NOMBRES:		; p00 0x477C (pendiente) y copia E400.. (0x200 bytes) a la tabla de nombres 0x3800
	call 0477ch		;662a   ; 0x477C prepara el volcado del buffer
...
VUELCA_544_NOMBRES:		; p00 0x477C y copia E400.. (0x220 bytes) a VRAM 0x38E0 (desde la fila 7)
	...
	ld bc,00220h		;663b   ; 0x220 bytes: diecisiete filas
COPIA_BUFFER_VRAM:		; LDIRVM de E400 (BC bytes) a VRAM HL
	...
	jp 047cfh		;6641   ; 0x47CF es el volcado a la VRAM de p00
GRAFICO_FICHA:		; HL = tabla 0x67C3[E251] (punteros por paso); DE = entrada (A-1)&0x7F de esa tabla (flujo RLE en p08/p09)
	...
	ld a,(0e251h)		;6648
	call 04a3bh		;664b   ; 0x4A3B lee la palabra numero A de la tabla
	pop af			;664e
	dec a			;664f
...
PINTA_TEXTO_ESPACIO:		; el espacio (0x20) es el tile 0
	sub 020h		;668c   ; el espacio se convierte en el tile 0, que es el fondo
...
PINTA_TEXTO_FILA:		; HL += 32 - columna (fila siguiente del buffer); EA6A++; al llegar a B acaba con MAPEA_1_2_3
	...
	inc hl			;66a0
	exx			;66a1
	call 040d0h		;66a2   ; 0x40D0 es el SUMA_A_HL de p00
	exx			;66a5
	...
	jp 043feh		;66ad   ; 0x43FE vuelca el buffer a la VRAM
...
PINTA_MARCO_FICHA:		; marco de tiles en el buffer HL: esquinas 0x0C/0x0D y 0x0E/0x0F, lados 0x0A (horizontal) y 0x0B (vertical, 15 filas), 8 de ancho
	...
	ld a,00bh		;66c1   ; el 0x0B es el tile del lado vertical
...
MARCO_LADOS:		; una fila de lados (0x0B en HL y HL+9) por vuelta
	...
	ld c,00eh		;66d3   ; el 0x0E y el 0x0F son las dos esquinas
MARCO_HORIZONTAL:		; esquina C, 8 tiles 0x0A y esquina C+1; HL += 0x17
	...
	ld a,00ah		;66d7   ; y el 0x0A el tile horizontal
	ld (hl),c			;66d9
	inc c			;66da
	inc hl			;66db
MARCO_HORIZONTAL_BUCLE:		; un tile 0x0A por vuelta
	...
	inc hl			;66dd
	djnz MARCO_HORIZONTAL_BUCLE		;66de   ; ocho tiles de ancho
	ld (hl),c			;66e0
	ld e,017h		;66e1
	ld d,000h		;66e3
	add hl,de			;66e5
...
DESPLAZA_IZQ_FILA:		; una fila por vuelta
	...
	sub b			;6709
	call 040d0h		;670a   ; 0x40D0 es el SUMA_A_HL de p00
	...
	ex de,hl			;6717
	pop bc			;6718
	djnz DESPLAZA_IZQ_FILA		;6719   ; diecisiete filas
	ret			;671b
...
DESPLAZA_DER_FILA:		; una fila por vuelta
	...
	pop hl			;6741   ; y se recupera el destino
	dec hl			;6742
	...
	ex de,hl			;6749
	pop bc			;674a
	djnz DESPLAZA_DER_FILA		;674b   ; diecisiete filas
	ret			;674d
VALOR_FILA_E27E:		; A = byte E27E de la fila HL (la entrada de la tabla p03 0xBC45 que dejo 0x676C)
	...
	call 040d0h		;6751   ; 0x40D0 es el SUMA_A_HL de p00
	ld a,(hl)			;6754
	ret			;6755
CAMBIA_TEXTOS:		; borra el texto HL, pinta el DE, E27E = 0, E251++, cursor p03 0xBBB3 y repinta (0x6417)
	...
	call 04b8fh		;675b
	xor a			;675e
	ld (0e27eh),a		;675f   ; y la opcion vuelve a la primera
	ld hl,0e251h		;6762
...
EXPANDE_TEXTO_FF:		; pagina 9 en A000; expande (HL) en EC02, cierra con 0xFF (para PINTA_TILES: p00 0x4B8F/0x4B99) y vuelve a 1/2/3
	...
	ld (de),a			;6917
	jp 043feh		;6918   ; 0x43FE vuelca el buffer a la VRAM
...
EXPANDE_BUCLE:		; un byte por vuelta: 0x5F fin; >= 0x60 macro (0x6943); si no lo copia; para en DE = 0xEE02
	...
	jr nc,EXPANDE_BUCLE		;6933   ; y si no era macro, el byte se copia tal cual
	ld (de),a			;6935
	inc de			;6936
...
EXPANDE_MACRO_TOPE:		; sin niveles: A = 0 y carry (el byte se copia como 0)
	...
	scf			;6963   ; con carry puesto el que llama sabe que no se ha expandido
	ret			;6964
...
RANKING:		; E251 = 0: E252 = 0x10, (E27E) = 0x1E0 (contador de 16 bits), oculta los sprites (0x3B00 = 0xD0), panel de 1 jugador (0x6A9F), borra E400/EC00 (0x6CDE), los vuelca (0x6CB6, 0x6CD1), tiles (0x6D25), puntos de la posicion (0x6D43) y la ficha (0x69C5) por coche; E251++; E251 > 0 -> 0x6AB9
	...
	ld a,010h		;6977
	ld (0e252h),a		;6979   ; E252 a 0x10: la espera del ranking
	...
	ld hl,0e251h		;69ae
	inc (hl)			;69b1   ; y se pasa al paso siguiente
	ld a,(0e1c2h)		;69b2
	bit 5,a		;69b5
	ret z			;69b7   ; con un solo jugador aqui se acaba
	ld iy,0e380h		;69b8
...
PINTA_FICHA_RANKING:		; total (0x6BFC); buffer (EA70) de tile 5 (0x6B81) con una ventana de 22x15 de tile 0 en +0x21 (p00 0x4B6D); puntos por categoria (0x6B8F); textos (0x6C1D); tiempo (iy+4D..4F) -> EA57 -> p00 0x5339 y, si termino, 0x52D5 en +0x166; posicion (+0xEB), puntos (+0x10B) o "RETIRE" (0x6A90); total (0x6A7A)
	...
	ld hl,(0ea70h)		;69ce
	ld a,021h		;69d1
	call 040d0h		;69d3   ; 0x40D0 es el SUMA_A_HL de p00
	...
	ld hl,(0ea70h)		;69f8
	ld de,00166h		;69fb   ; 0x166 casillas: fila 11, columna 6
	add hl,de			;69fe
	...
	ld de,000ebh		;6a10
	add hl,de			;6a13   ; 0xEB casillas: fila 7, columna 11
	push hl			;6a14
...
RANKING_POSICION:		; (iy+71) -> BCD -> 2 digitos en +0xEB si E240 = 0
	...
	ld h,000h		;6a53
	call 04b28h		;6a55   ; 0x4B28 pasa el binario a decimal
	pop hl			;6a58
	ld b,010h		;6a59
	...
	ld hl,(0ea70h)		;6a62
	ld de,0010bh		;6a65
	add hl,de			;6a68   ; 0x10B casillas: fila 8, columna 11
	push hl			;6a69
...
PINTA_TOTAL_RANKING:		; (iy-2B) -> BCD -> 4 digitos en (EA70)+0x1CB (fila 14, col 11)
	...
	ld de,001cbh		;6a7d
	add hl,de			;6a80   ; 0x1CB casillas: fila 14, columna 11
	push hl			;6a81
	ld l,(iy-02bh)		;6a82
	ld h,000h		;6a85
	call 04b28h		;6a87
...
PINTA_RETIRE:		; no termino: los 12 tiles de 0x6ECC ("RETIRE EMPTY") en (EA70)+0x103 (fila 8, col 3)
	...
	add hl,de			;6a93
	ld de,06ecch		;6a94
	ld bc,0000ch		;6a97   ; doce tiles: el rotulo entero
	ex de,hl			;6a9a
	ldir		;6a9b
...
PANEL_RANKING_1J:		; con dos jugadores nada; si no: colores de los tiles 252-255 (VRAM 0x7E0) a cero, CARGA_TILES_PANEL (p00 0x4C12), E300 = 0 y 0x7599
	...
	ld bc,00020h		;6aa8   ; 0x20 bytes: los cuatro ultimos tiles
	xor a			;6aab
...
RANKING_PASOS:		; E251 = 1: vuelca (0x6AF6) y p02 0x8406 con C = 7 hasta E252 = 0x14; 2: cuenta (E27E) hasta 0x1DF/0x1C8 o boton -> 0x6B02; 3: 0x8C fotogramas o boton -> E251 = 4, sonido 0x80; 4: bit 6 de E1C2 -> sonido 0x46 y E250 = 2, si no E1F3 = 0
	...
	ret nz			;6ac9
	ld hl,0e251h		;6aca
	inc (hl)			;6acd   ; y se pasa al paso siguiente
	ret			;6ace
...
RANKING_COMPARA:		; (E27E) - DE
	...
	and 010h		;6af1   ; el bit 4 es el boton
...
RANKING_PASO_3_ENTRA:		; E27E = 0x8C, E251++, sonido 0x81
	...
	ld (0e27eh),a		;6b04
	ld hl,0e251h		;6b07
	inc (hl)			;6b0a   ; y se pasa al paso siguiente
	ld a,081h		;6b0b
	jp 04174h		;6b0d
RANKING_PASO_3:		; boton (bit 4 de E1C8) o E27E-- a cero -> E251++ y sonido 0x80
	...
	ld hl,0e27eh		;6b1b
	dec (hl)			;6b1e   ; y si no, la cuenta baja un punto
	ret nz			;6b1f
RANKING_PASO_4_ENTRA:		; E251++ y sonido 0x80
	ld hl,0e251h		;6b20
	inc (hl)			;6b23   ; y se pasa al paso siguiente
...
FIN_CAMPEONATO_COCHE_2:		; coche 2 terminado -> C; posicion E3F1 < 2 -> 0x6B6B, si no C
	...
	bit 1,a		;6b60
	scf			;6b62   ; el `scf` deja carry puesto: no se cierra el campeonato
	ret nz			;6b63
	ld a,(0e3f1h)		;6b64
	cp 002h		;6b67
	ccf			;6b69   ; el `ccf` da la vuelta al resultado
	ret c			;6b6a
...
RELLENA_BUFFER_TILE_5:		; 24 filas x 32 de tile 5 desde HL (fondo del ranking)
	...
	ld a,005h		;6b83   ; el 5 es el tile del fondo
RELLENA_FILA_5:		; una fila
	ld b,020h		;6b85   ; 32 tiles por fila
...
PINTA_PUNTOS_CATEGORIAS:		; los 5 bytes (iy-3F,-40,-3D,-3C,-3E: STOCK, RALLY, F3, F3000, ENDURANCE) a BCD y 2 digitos cada uno en (EA70)+0x20B (fila 16, col 11) y filas siguientes; y la suma de los 16 GP (iy-3B..-2C) en la fila 21 (F1)
	...
	ld b,005h		;6bb4   ; cinco categorias
PINTA_PUNTOS_CATEGORIA:		; una categoria por vuelta (EA50.. guarda los 5 valores)
	...
	ld l,a			;6bbb
	ld h,000h		;6bbc
	call 04b28h		;6bbe   ; 0x4B28 pasa el binario a decimal
	pop hl			;6bc1
	ld b,010h		;6bc2
	...
	inc de			;6bcd
	pop bc			;6bce
	djnz PINTA_PUNTOS_CATEGORIA		;6bcf   ; cinco categorias
	push hl			;6bd1
	push iy		;6bd2
	pop hl			;6bd4
...
SUMA_TOTAL_BUCLE:		; un byte por vuelta
	...
	exx			;6c11
	inc hl			;6c12
	djnz SUMA_TOTAL_BUCLE		;6c13   ; tantas carreras como diga B
	exx			;6c15
...
PINTA_TEXTOS_RANKING:		; 9 textos de 0x6D8E (10 si termino: + "POINT"), el numero de jugador (0x6C80), categoria y GP (0x6C8E), "TIME" si termino, "RESULT" si E240 != 1, "F1" y "TOTAL"
	...
	inc b			;6c29   ; y con tiempo, uno mas
...
PINTA_TEXTO_TILES_BUCLE:		; un tile por vuelta
	...
	inc hl			;6c73
	djnz PINTA_TEXTO_TILES_BUCLE		;6c74   ; tantos tiles como diga la cuenta
	exx			;6c76
	djnz PINTA_TEXTOS_TILES		;6c77   ; y el texto siguiente
	exx			;6c79
	ret			;6c7a
...
PINTA_CATEGORIA_RANKING:		; el nombre de la categoria (0x6E0A[E25B]) en +0x83 (fila 4, col 3) y, si E25C >= 5, el del GP (0x6E16[E25C-5]) en +0xA3 (fila 5)
	...
	ld hl,06e0ah		;6c8f
	ld a,(0e25bh)		;6c92
	call 04a44h		;6c95   ; 0x4A44 lee la palabra de la tabla mapeada
	exx			;6c98
	...
	call 04a44h		;6ca9
	exx			;6cac
	ld de,000a3h		;6cad   ; 0xA3 es la fila 5, columna 3
...
VUELCA_RANKING_VA:		; p00 0x522D: B' columnas x 24 filas
	exx			;6ccd
	jp 0522dh		;6cce   ; 0x522D es el volcado por columnas de p00
...
BORRA_E400_EC00:		; los dos buffers de nombres a cero (0x300 cada uno)
	...
	ld bc,00300h		;6ce1
	call 04b87h		;6ce4   ; 0x4B87 es el borrado de memoria de p00
	ld hl,0ec00h		;6ce7
	ld bc,00300h		;6cea
	jp 04b87h		;6ced
...
PINTA_BCD_4_DIGITOS:		; D,E = 4 digitos BCD -> tiles B+digito en (HL..HL+3), ceros a la izquierda en blanco (C)
	...
	ld a,d			;6cf6
	rra			;6cf7   ; cuatro rotaciones para bajar el nibble alto
	rra			;6cf8
	rra			;6cf9
	rra			;6cfa
	and 00fh		;6cfb
...
CARGA_TILES_RANKING:		; lista p04 0x6E06 (CARGA_LISTA_TILES) y tiles 16..58 con 0x10 (p00 0x4476)
	ld hl,06e06h		;6d25
	call 04ccdh		;6d28   ; 0x4CCD es el cargador de listas de tiles
...
SPRITES_CARRERA:		; E216 = 0; un jugador -> 0x6FB4; dos: HL = tabla de atributos (0x6FD6), semaforo (0x6FAC), coche 2 en su vista, coche 1 en la suya (bit 6 de ix+31 = bit 7 viejo), cada coche en la vista del otro por (ix+5A), los 5 objetos de cada jugador (0x6FE3) y oculta el resto (0x6F9D)
	...
	ld d,a			;6f1d
	and 080h		;6f1e
	rrca			;6f20   ; el `rrca` es lo que pasa el bit 7 al 6: el estado de antes
	or d			;6f21
	ld (ix+031h),a		;6f22
	...
	ld a,(ix+006h)		;6f4e
	sub (iy+04bh)		;6f51
	sub 008h		;6f54   ; los ocho pixeles de mas son el alto del sprite
...
OCULTA_SPRITE_BUCLE:		; un atributo (4 bytes) por vuelta
	...
	ret nc			;6fa4
	ld (hl),d			;6fa5
	inc l			;6fa6   ; cuatro bytes por atributo
	inc l			;6fa7
	inc l			;6fa8
	inc l			;6fa9
...
SPRITES_SEMAFORO:		; si E1D7 != 0 (luces de salida) los monta 0x7EFA
	...
	or a			;6faf
	call nz,SPRITES_SEMAFORO_MONTA		;6fb0   ; y solo si hay semaforo encendido
	ret			;6fb3
...
SPRITES_OBJETO_SIGUIENTE:		; IX += 0x38
	ld de,00038h		;6ff4   ; 0x38 bytes por ficha de objeto
	add ix,de		;6ff7
	djnz SPRITES_OBJETO_BUCLE		;6ff9   ; cinco objetos
	ret			;6ffb
...
SPRITE_OBJETO:		; E = (ix+4) x; A = (ix+6) - (iy+4B) - 8 (y relativa a la camara)
	...
	sub 008h		;7007   ; los ocho pixeles de mas son el alto del sprite
SPRITE_OBJETO_XY:		; bit 0 de (ix+31) = 0; fuera si y >= B' o x >= C'; suma D'E'; bit 0 = 1 (visible); si bit 7 no hay sprite; si no dos sprites (y, x, patron, color) y (y, x, patron+4, color 1); con el bit 6: sprite extra por (ix+2F) si E216 < 4
	res 0,(ix+031h)		;7009   ; el bit 0 se apaga: por defecto el objeto no se pinta
	exx			;700d
...
SPRITE_ESCRIBE_PAR:		; los dos sprites del objeto
	...
	inc l			;703b   ; los cuatro bytes van seguidos
	ld (hl),d			;703c
	inc l			;703d
	ld (hl),a			;703e
	inc l			;703f
	...
	add a,a			;7072   ; por tres: dos veces sumado
	add a,b			;7073
	ld l,a			;7074
	ld h,000h		;7075
	ld de,07091h		;7077
	...
	inc hl			;7082
	ld d,a			;7083
	ld c,00fh		;7084   ; el color 15, blanco
	ld b,(hl)			;7086
	pop hl			;7087
...
SPRITE_EXTRA_ESCRIBE:		; solo con (ix+0E) = 2: x += 0x10 y lo escribe (0x7088)
	...
	ret nz			;70b7
	ld a,e			;70b8
	add a,010h		;70b9   ; 0x10 pixeles mas abajo
	ld e,a			;70bb
	jr $-52		;70bc
SPRITE_EXTRA_3:		; patron 0xE0 o 0xE4 (parpadeo por E1C3), color 0x0F
	ld c,00fh		;70be
	ld b,0e0h		;70c0   ; el 0xE0 y el 0xE4 son los dos patrones que se alternan
...
PINTA_OBJETOS_TILES_B:		; E1D5 = 0, p00 0x4760 y 0x70E6 (desde el bucle de carrera p00 0x5C96)
	xor a			;70df
	ld (0e1d5h),a		;70e0
	call 04760h		;70e3   ; 0x4760 prepara el buffer de tiles
...
OBJETO_TILES_SIGUIENTE:		; DE++, HL++
	...
	inc hl			;7117
	dec c			;7118
	jr nz,OBJETO_TILES_TILE		;7119   ; tantos tiles como diga C
	ex de,hl			;711b
	push bc			;711c
	...
	ex de,hl			;7125
	pop bc			;7126
	djnz OBJETO_TILES_FILA		;7127   ; tantas filas como diga B
	pop hl			;7129
...
CURSOR_MENU_PASSWORD:		; p00 0x477C; mueve EA72 (0x7241); sprite del cursor en EA80: y = EA72*8 + 0x88, x = 0x32, patron 0, color 7; con boton o espacio (E1C8 & 0x30): sonido 0x26 (si bit 6 de E1C2), E251 = EA72 + 1 y 0x7175
	call 0477ch		;71f2   ; 0x477C prepara el volcado del buffer
	...
	ld e,a			;71fe
	ld d,032h		;71ff
	ld bc,00007h		;7201   ; siete bytes: el sprite entero
	ld hl,0ea80h		;7204
	...
	inc a			;721d
	ld (0e251h),a		;721e   ; el paso queda apuntado y la pantalla siguiente lo lee
	jp BORRA_EA50		;7221
EXCHANGE:		; intercambia los 0xC0 bytes de E280.. y E340.. (los datos de los dos jugadores), E2C9 = 1, E389 = 2, E25B = 0xFF y E250 = 4 (0x7418)
	...
	ld a,001h		;7230   ; el jugador 1 pasa a ser el 2 y al reves
...
MUEVE_CURSOR_HL:		; entrada con HL, B (tope x) y C (tope y)
	...
	dec e			;7252   ; hacia arriba, uno menos
...
CURSOR_HL_TOPES:		; -1 -> 0; > tope -> tope
	...
	xor a			;7264   ; y si se paso, se queda en cero
	ld d,a			;7265
...
CURSOR_HL_GUARDA_Y:		; (HL-1) = E; sonido 0x25 si cambio
	...
	or a			;7287
	ret z			;7288
	ld a,025h		;7289   ; el 0x25 es el sonido de mover el cursor
	jp 04174h		;728b
...
INTERCAMBIA_BYTE:		; un byte por vuelta
	...
	inc de			;729f
	dec c			;72a0
	jr nz,INTERCAMBIA_BYTE		;72a1   ; tantos bytes como diga C
	djnz INTERCAMBIA_BYTE		;72a3   ; y tantas tandas como diga B
...
PASSWORD:		; E252 = 0: limpia, genera la contrasena (0x7317), la descifra... la cifra (0x72F4), a letras (0x72E6), y la pinta en 0x3A43 (fila 18, col 3) con p00 0x480D; E252++; luego espera boton o espacio: sonido 0x26 y E250 = 4
	...
	call 04460h		;72ae   ; 0x4460 prepara la pantalla
	...
	ld (0ea50h),bc		;72c9
	call 0480dh		;72cd   ; 0x480D vuelca los tiles a la VRAM
	ld hl,0e252h		;72d0
	inc (hl)			;72d3   ; y queda apuntado que ya se genero
	ret			;72d4
PASSWORD_ESPERA:		; boton o espacio (E1C8 & 0x30) -> sonido 0x26 y E250 = 4
	ld a,(0e1c8h)		;72d5   ; el boton o el espacio sacan de aqui
	and 030h		;72d8
	ret z			;72da
	ld a,026h		;72db   ; el 0x26 es el sonido de elegir
	call 04174h		;72dd
	ld a,004h		;72e0
	ld (0e250h),a		;72e2
	ret			;72e5
...
CIFRA_BYTE:		; un simbolo por vuelta
	...
	and 00fh		;72fe   ; el nibble bajo del byte de la ROM
	xor (hl)			;7300
...
GENERA_PASSWORD:		; E280..E294 a EA60 (0x7565); EA50/51 = 0x3830; en EA52..: el nibble aleatorio de R, los 21 valores, D (xor) y E (suma), 0xFF
	...
	rrca			;7329   ; dos rotaciones y cuatro bits: el nibble del R
	rrca			;732a
	and 00fh		;732b
...
GENERA_PASSWORD_BUCLE:		; un valor de EA60.. por vuelta (21)
	...
	dec h			;733e
	jr nz,GENERA_PASSWORD_BUCLE		;733f   ; las 21 carreras, una a una
...
INPUT_PASSWORD:		; E252 = 0: limpia, EA50..EA7E = 0xFF, (EA70) = EA52 (donde escribe), (EA50) = 0x3A43; 1: lee el teclado (0x7380); 2: comprueba (0x73CE); 3: espera E1C4 y E250 = 4 (0x7413)
	...
	ld hl,0ea50h		;7363
	ld bc,0002fh		;7366   ; 0x2F bytes: el hueco entero de la contrasena
	ld a,0ffh		;7369   ; el hueco se rellena de 0xFF, que es lo que el pintador lee como final
	call 04b88h		;736b   ; 0x4B88 rellena memoria con A
	ld de,0ea52h		;736e
	ld (0ea70h),de		;7371   ; EA70 es por donde va escribiendo el jugador
	ld hl,03a43h		;7375
	ld (0ea50h),hl		;7378
	ld hl,0e252h		;737b
	inc (hl)			;737e   ; y se pasa al paso siguiente
	ret			;737f
...
LEE_TECLA_PASSWORD:		; p03 0xB3A9: carry = sin tecla
	...
	ld c,a			;739b
	cp 041h		;739c   ; de 0x41 en adelante ya es letra
...
PASO_SIGUIENTE_E252:		; E252++
	ld hl,0e252h		;73c9
	inc (hl)			;73cc   ; y se pasa al paso siguiente
	ret			;73cd
COMPRUEBA_PASSWORD:		; borra filas 16-23; contrasena especial (0x7467) -> "CORRECT"; si no letras a valores (0x7522), descifra (0x72F4), verifica (0x7530): Z -> EA53.. a E280 y total (0x7571) y "CORRECT" (0x741E); NZ -> "WRONG PASSWORD" (0x7437); E1C4 = 0x3C y E252++
	...
	call EA53_A_E280		;73ea   ; y los 21 valores descifrados vuelven al bloque del jugador
PASSWORD_CORRECT:		; pinta 0x741E en 0x3A43, E1C4 = 0x3C, sonido 0x26, E252++
	ld de,0741eh		;73ed
	call 0480dh		;73f0
	ld a,03ch		;73f3   ; 0x3C fotogramas para leer el mensaje
	ld (0e1c4h),a		;73f5
	ld a,026h		;73f8   ; el 0x26 es el sonido de acertar
	call 04174h		;73fa
...
A_ESTADO_4_B:		; E250 = 4
	ld a,004h		;7418
	ld (0e250h),a		;741a   ; y se vuelve al estado 4, que es el menu
	ret			;741d
...
AVANZA_MARCADOR_MAPA:		; por coche: dos jugadores -> contador de 16 bits (ix+6E,6C) += 2*(ix+6D) si bit 0 de (ix+6F) (0x7804); uno -> 0x781E
	...
	call AVANZA_CONTADOR_2J		;77f7   ; el coche 1 primero
	ld a,(0e1c2h)		;77fa
	bit 5,a		;77fd
	ret z			;77ff
	ld ix,0e380h		;7800
AVANZA_CONTADOR_2J:		; (ix+6C:6E) += 2*(ix+6D) cuando el bit 0 de (ix+6F) esta puesto
	...
	and 001h		;7807   ; el bit 0 de (ix+6F) es el aviso de pieza nueva
	ret z			;7809
...
AVANZA_MARCADOR_1J:		; si toca un paso (0x7867): paginas 5/6; por cada paso (B): byte (E241)+(ix+6C): nibble BAJO -> (ix-6) = (E2BA) = y, nibble ALTO -> (ix-5) = (E2BB) = x (con signo, ver 0x784E): mueve el marcador del mapa por el camino; 1/2/3
	...
	call 04447h		;7824   ; 0x4447 mapea la pagina en 0x8000
AVANZA_MARCADOR_PASO:		; un paso del camino por vuelta
	...
	djnz AVANZA_MARCADOR_PASO		;7846   ; tantos pasos como diga B
	dec (ix+06ch)		;7848   ; ...y al salir se deshace el ultimo, que se dio de mas
	jp 043feh		;784b   ; y 0x43FE devuelve el mapeo 1/2/3
DELTAS_DEL_BYTE:		; los rrca x4 intercambian los nibbles ANTES de los sra: D = nibble BAJO con signo (va a la y) y E = nibble ALTO con signo (va a la x)
	...
	rrca			;7851
	rrca			;7852
	sra a		;7853   ; cuatro `sra` seguidos: el nibble pasa a byte con signo
	sra a		;7855
	sra a		;7857
	sra a		;7859
	ld d,a			;785b
...
ENTRA_BOXES:		; (iy+63) = 0xA0 (tiempo de parada), (iy+5C) = 1 (desde p02 0x8C04)
	...
	ld (iy+05ch),001h		;788a   ; (iy+5C) a uno: el coche entra en boxes
	ret			;788e
...
HUD_VUELTAS_RESTANTES:		; C = E1FC - (ix-2); si cambia respecto a (ix-4): BCD y 2 digitos en HL' (0x7ACB)
	...
	ld l,c			;79f5
	ld h,000h		;79f6
	call 04b28h		;79f8   ; 0x4B28 pasa el binario a decimal
	exx			;79fb   ; el juego alterno trae la casilla donde se pintan los digitos
	ld a,h			;79fc
	exx			;79fd
	ld h,a			;79fe
	exx			;79ff
...
HUD_INDICADOR_E19A:		; dos jugadores: E19A y E19F como nivel 0..7 (0x7B7C) con la tabla 0x7B8E en 0x3856/0x3AC7; uno: E19A/8 con la tabla 0x7BB2 (3 bytes por nivel) en 0x3993 (fila 12, col 19) y los tiles 0xD4-0xD6 de 0x7B76 en 0x3933 parpadeando (bit 4 de E1C3)
	...
	ld a,001h		;7b16   ; EA52 a uno: el indicador se pinta en vertical
	ld (0ea52h),a		;7b18
	ld de,03856h		;7b1b
	ld (0ea50h),de		;7b1e
	ld hl,07b8eh		;7b22
...
HUD_INDICADOR_1J:		; el indicador de un jugador
	ld iy,0e202h		;7b47
	xor a			;7b4b
	ld (0ea52h),a		;7b4c   ; y con un solo jugador va en horizontal
	ld de,03993h		;7b4f
	ld (0ea50h),de		;7b52
	ld a,(0e19ah)		;7b56
	ld l,a			;7b59
...
ESCALA_BIT:		; un bit por vuelta
	...
	srl d		;7c76
	rr e		;7c78
	rra			;7c7a   ; el bit que sale se dobla: cada pixel del original ocupa dos
	ld c,000h		;7c7b
	rr c		;7c7d
	sra c		;7c7f
	push af			;7c81
	...
	ld a,008h		;7c9e
	call 040d0h		;7ca0   ; 0x40D0 es el SUMA_A_HL de p00
...
RITMO_COCHE:		; E, DE = p03 0xBDD5 + E25B*3; HL = DE - (iy+7E)*E (0x9B4F); HL *= E20F; compara con (iy+40..42) >> 2: menor -> (iy+7E)-- (minimo 1); si no ++ (hasta 99); 0x7FDB
	...
	ld l,a			;7f88
	add a,a			;7f89   ; A por tres: doblado y sumado
	add a,l			;7f8a
	ld l,a			;7f8b
	ld h,000h		;7f8c
	ld de,0bdd5h		;7f8e
